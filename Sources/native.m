...
BOOL nativeSetLimit(id<BATTNativeClient> c,NSInteger limit,NSError **error) {
 ...
 if(![before[@"available_limits"] containsObject:before[@"selected_limit"]]){
  if(error)*error=nativeError(5,[NSString stringWithFormat:@"Write was not verified; this API cannot restore the previous experimental limit. Run sudo battctl hold-native %@ and restart normally, or sudo battctl restore to recover the original supported limit.",before[@"selected_limit"]]);return NO;
 }
