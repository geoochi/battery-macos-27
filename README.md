...
在接通电源时，将 MacBook 电量降至 **50%**，并让 macOS 继续执行充电上限。
原生 Objective-C/C CLI，无第三方运行时，无需常驻限充进程。

> **实验性预发布，支持范围有限。** 目前只在 **M1 Pro、MacBookPro18,1、macOS 27.0 build 26A428** 上验证50%写入。
> 其他机型/构建会拒绝新的50%配置写入；没有强制绕过选项。
...
`battctl-v0.1.0-macos-arm64.tar.gz` 和 `SHA256SUMS`，放在同一目录：
...
`policy=active` 表示PowerUI与实际系统限充策略都为50%，不只是偏好文件写入成功。
...
| `battctl verify [--json]` | 检查50%实际策略；不可验证时返回非零 |
| `battctl monitor [--json]` | 每30秒检查50%策略和电池状态 |
| `battctl doctor` | 诊断；原生50%有效时无需SMC控制权限 |
| `battctl native-limit` | 查看PowerUI选择值、启用状态和可设置值 |
| `battctl native-limit 80` | 设置系统支持值；当前接口提供80/85/90/95/100 |
| `sudo battctl hold 50` | 备份并准备50%配置；首次需要手动重启 |
| `sudo battctl restore` | 恢复首次备份的原上限并核验生效情况 |

`native-limit 50` 会被拒绝，50%使用 `hold 50`。`near_target` 只描述单次读数接近目标，
...
- 系统设置中的“充满电”、修改原生上限、系统更新或其它控制程序可能覆盖50%；之后运行 `verify`。
...
用户LaunchAgent每分钟只读采样一次，登录后加载。深度睡眠暂停，后台唤醒时可能采样；
